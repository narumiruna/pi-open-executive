...
const packageDir = path.dirname(path.dirname(extensionPath));
...
function registerChildGuard(pi: ExtensionAPI): void {
	pi.on("tool_call", (event, ctx) => {
		...
		const input = (event.input && typeof event.input === "object" ? event.input : {}) as {
			path?: unknown;
		};
...
export default function openExecutive(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptSnippet:
			"Consult specialist department heads for substantive or cross-functional business decisions",
		promptGuidelines: [
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			const results = outcomes
				.filter(
					(outcome): outcome is PromiseFulfilledResult<ConsultationResult> =>
